...
typedef struct
{
  ...
  /* Regexp search part. */
  ecma_value_t regexp; /**< regular expression */

  /* String search part. */
  lit_utf8_iterator_t search_iterator; /**< string pattern iterator */
  lit_utf8_iterator_t input_iterator; /**< input iterator */
  ecma_value_t search_string;
...
ecma_builtin_string_prototype_object_replace_match (replace_search_context *context_p) /**< search context */
{
  ...
  if (context_p->is_regexp)
  {
    ...
    {
      ecma_value_t object = ecma_get_completion_value_value (ret_value);

      if (!ecma_is_value_null (object))
      {
        JERRY_ASSERT (ecma_is_value_object (object));

        ecma_completion_value_t nested_ret_value = ecma_make_empty_completion_value ();
        ...
        ECMA_TRY_CATCH (index_value,
                        ecma_op_object_get (object_p, index_string_p),
                        nested_ret_value);

        ECMA_TRY_CATCH (result_string_value,
                        ecma_op_object_get (object_p, zero_string_p),
                        nested_ret_value);
        ...
        if (!ecma_is_completion_value_empty (nested_ret_value))
        {
          /* Error occured when the indicies are read. */
          ecma_free_completion_value (ret_value);
          ret_value = nested_ret_value;
        }
      }
    }
  }
  else
  {
    ...
    JERRY_ASSERT (lit_utf8_iterator_is_bos (&context_p->input_iterator));
    JERRY_ASSERT (lit_utf8_iterator_is_bos (&context_p->search_iterator));

    ecma_length_t match_start = 0;
    ecma_length_t match_end = 0;
    bool match_found = false;

    if (lit_utf8_iterator_is_eos (&context_p->search_iterator))
    {
    ...
    {
      ecma_char_t first_char = lit_utf8_iterator_read_next (&context_p->search_iterator);

      while (!lit_utf8_iterator_is_eos (&context_p->input_iterator))
      {
        if (lit_utf8_iterator_read_next (&context_p->input_iterator) == first_char)
        {
          /* Local copy to preserve their original value. */
          lit_utf8_iterator_t search_iterator = context_p->search_iterator;
          lit_utf8_iterator_t input_iterator = context_p->input_iterator;
          match_end = match_start + 1;

          match_found = true;
          while (!lit_utf8_iterator_is_eos (&search_iterator))
          {
            if (lit_utf8_iterator_is_eos (&input_iterator))
            {
              match_found = false;
              break;
            }

            if (lit_utf8_iterator_read_next (&input_iterator) != lit_utf8_iterator_read_next (&search_iterator))
            {
              match_found = false;
              break;
            }
    ...
    if (match_found)
    {
      ecma_value_t arguments_list_p[1] = { context_p->search_string };
      ECMA_TRY_CATCH (new_array_value,
                                                         ...
                                                         ecma_value_t match_value) /**< returned match value */
{
  ...
  else
  {
    ...
    while (!lit_utf8_iterator_is_eos (&replace_iterator))
    {
      ...
      else if (action == LIT_GRAVE_ACCENT)
      {
                                              ...
                                              ecma_value_t replace_value) /**< routine's second argument */
{
      ...
      && ecma_object_get_class_name (ecma_get_object_from_value (search_value)) == LIT_MAGIC_STRING_REGEXP_UL)
  {
    ...
    context.input_string = to_string_val;
    context.input_length = ecma_string_get_length (ecma_get_string_from_value (to_string_val));
    context.regexp = search_value;
  ...
  else
  {
    ...
    ecma_string_t *search_string_p = ecma_get_string_from_value (to_string_search_val);
    ...
    context.input_string = to_string_val;
    context.input_length = ecma_string_get_length (ecma_get_string_from_value (to_string_val));
    context.search_iterator = lit_utf8_iterator_create (search_start_p, search_size);
    context.input_iterator = lit_utf8_iterator_create (input_start_p, input_size);
    context.search_string = to_string_search_val;
